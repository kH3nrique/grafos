...
void atualizaTempo(struct Jogo* jogo, const char* localA, const char* localB) {
    ...
    if (jogo->matrizAdjacencia[indiceA][indiceB] == 0) {
        int novoPeso;
        cout << "A movimentacao entre " << localA << " e " << localB << " nao esta inserida." << endl;
        cout << "Insira o peso (movimentacao de carga) entre " << localA << " e " << localB << ": ";
        cin >> novoPeso;

        // Atualiza a matriz de adjacência com o novo peso (tempo de deslocamento)
        jogo->matrizAdjacencia[indiceA][indiceB] = novoPeso;
        jogo->matrizAdjacencia[indiceB][indiceA] = novoPeso;

        cout << "Movimentacao de carga entre " << localA << " e " << localB << " atualizada com sucesso." << endl;
    } else {
        cout << "Movimentacao de carga entre " << localA << " e " << localB << " ja existe." << endl;
    }
}
...
void executarMenuJogo(struct Jogo* jogo) {
    int opcao;
